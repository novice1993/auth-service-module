...
export const selectNecessaryData = (targetObject: TargetObjectType, keyNames: { [key: string]: string }) => {
  ...
  Object.keys(keyNames).forEach(keyName => {
    ...
    authInfo[keyName] = findValueByKey(targetObject, authInfoKey) as string | number | boolean | undefined;
  });

  return Object.values(authInfo).some(value => value !== undefined) ? authInfo : null;
};
